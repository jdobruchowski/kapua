...
public class AmqpClient {
    ...
    public AmqpClient(ProtonSession session, ClientOptions clientOptions) {
        ...
        logger.info("AAAAA-Created client {} for session {}", clientId, session);
        validate();
    }
    ...
    public void send(Message message, String destination) throws KapuaException {
        ...
        if (operationSucceed && sender.isOpen()) {
            logger.info("RRRSSS-Acquired open sender after: {}ms DONE", (System.currentTimeMillis() - startTime));
        }
        else {
            logger.info("RRRSSS-Acquired open sender after: {}ms FAILED", (System.currentTimeMillis() - startTime));
            notifySenderError();
        }
        logger.info("RRRSSS-Sender ready after: {}ms", System.currentTimeMillis() - startTime);
        message.setAddress(destination);
        sender.send(message, delivery -> {
            logger.info("RRRSSS-MESSAGE SENT!!!! {} - {}", message);
        });
    }
    ...
    public void subscribe(String destination, ProtonMessageHandler messageHandler) throws KapuaException {
        ...
        if (operationSucceed && receiver.isOpen()) {
            logger.info("RRRSSS-Acquired open receiver after: {}ms DONE", (System.currentTimeMillis() - startTime));
        }
        else {
            logger.info("RRRSSS-Acquired open receiver after: {}ms FAILED", (System.currentTimeMillis() - startTime));
            notifyReceiverError();
        }
        logger.info("RRRSSS-Receiver ready after {}ms", System.currentTimeMillis() - startTime);
    }
    ...
    protected ProtonReceiver createReceiver(ProtonSession session, String destination, int prefetch, boolean autoAccept, ProtonQoS qos, CountDownLatch receiverCountDown, ProtonMessageHandler messageHandler) {
        ...
        try {
            logger.info("RRRSSS-Register consumer for destination {}... (session: {})", destination, session);
            ...
            logger.info("RRRSSS-Setting auto accept: {} - QoS: {} - prefetch: {}", autoAccept, qos, prefetch);

            receiver.handler(messageHandler);
            receiver.openHandler(ar -> {
                if(ar.succeeded()) {
                    logger.info("RRRSSS-Succeeded establishing consumer link! (session: {})", session);
                    if (receiverCountDown != null) {
                        receiverCountDown.countDown();
                    }
                }
                else {
                    logger.warn("RRRSSS-Cannot establish link! (session: {})", session, ar.cause());
                    notifyReceiverError();
                }
            });
            receiver.closeHandler(recv -> {
                logger.warn("RRRSSS-Receiver is closed! (session: {})", session, recv.cause());
                notifyReceiverError();
            });
            receiver.open();
            logger.info("RRRSSS-Register consumer for destination {}... OPENED (session: {})", destination, session);
        }
        catch(Exception e) {
            notifyReceiverError();
        }
    ...
    public void notifyReceiverError() {
        logger.info("RRRSSS-Receiver ERROR");
        invalidate();
    }

    public void notifySenderError() {
        logger.info("RRRSSS-Sender ERROR");
        invalidate();
    }
    ...
    protected ProtonSender createSender(ProtonSession session, String destination, boolean autoSettle, ProtonQoS qos, CountDownLatch senderCountDown) {
        ...
        try {
            logger.info("RRRSSS-Register sender for destination {}... (session: {})", destination, session);
            ...
            logger.info("RRRSSS-Setting auto accept: {} - QoS: {}", autoSettle, qos);

            sender.openHandler(ar -> {
               if (ar.succeeded()) {
                   logger.info("RRRSSS-Register sender for destination {}... DONE (session: {})", destination, session);
                   if (senderCountDown != null) {
                       senderCountDown.countDown();
                   }
               }
               else {
                   logger.info("RRRSSS-Register sender for destination {}... FAILED... (session: {})", destination, session, ar.cause());
                   notifySenderError();
               }
            });
            sender.closeHandler(snd -> {
                logger.warn("RRRSSS-Sender is closed! (session: {})", session, snd.cause());
                notifySenderError();
            });
            sender.open();
            logger.info("RRRSSS-Register sender for destination {}... OPENED (session: {})", destination, session);
        }
        catch(Exception e) {
            notifySenderError();
        }
...
//        protonSender.send(msg, deliveryHandler);
    ...
    public void clean() {
        ...
        if (sender!=null) {
            logger.info("AAAAA-Closing sender {} for session {}", sender, session);
            sender.closeHandler(ar -> {
                if(ar.succeeded()) {
                    logger.info("AAAAA-Sender close (session: {})", session, ar.cause());
                }
                else {
                    logger.warn("AAAAA-Sender close FAILED (session: {})", session, ar.cause());
                }
            ...
            sender.close();
            sender = null;
            logger.info("AAAAA-Closing sender for session {} DONE", session);
        }
        if (receiver!=null) {
            logger.info("AAAAA-Closing receiver {} for session {}", receiver, session);
            receiver.closeHandler(ar -> {
                if(ar.succeeded()) {
                    logger.info("AAAAA-Receiver close (session: {})", session, ar.cause());
                }
                else {
                    logger.warn("AAAAA-Receiver close FAILED (session: {})", session, ar.cause());
                }
                if (cleanCountDown!=null) {
                    cleanCountDown.countDown();
                }
            ...
            receiver.close();
            receiver = null;
            logger.info("AAAAA-Closing receiver for session {} DONE", session);
        }
        boolean operationSucceed = true;
        try {
            operationSucceed = cleanCountDown.await(AmqpClient.maxWait, TimeUnit.MILLISECONDS);
        } catch (InterruptedException e) {
            logger.debug("AAAAA-Wait interrupted");
        }
        if (operationSucceed) {
            logger.info("AAAAA-Cleaned receiver/sender {} after: {}ms {} DONE", this, (System.currentTimeMillis() - startTime), Thread.currentThread().getId(), Thread.currentThread().getName());
        }
        else {
            logger.info("AAAAA-Cleaned receiver/sender {} after: {}ms {} FAILED", this, (System.currentTimeMillis() - startTime), Thread.currentThread().getId(), Thread.currentThread().getName());
        }
        if (session != null) {
            session.closeHandler(event -> {
                if(event.succeeded()) {
                    logger.info("AAAAA-Session close (session: {})", session, event.cause());
                    session = null;
                }
                else {
                    logger.warn("AAAAA-Session close FAILED (session: {})", session, event.cause());
                }
            });
            session.close();
            logger.info("AAAAA-Closing session {} for session {} DONE", session, session);
            session = null;
        }
        try {
            operationSucceed &= cleanCountDown.await(AmqpClient.maxWait, TimeUnit.MILLISECONDS);
        } catch (InterruptedException e) {
            logger.debug("RRRSSS-Wait interrupted");
        }
        if (sender!=null && sender.isOpen()) {
            logger.warn("RRRSSS-Cleaned sender {} - {} after: {}ms FAILED (sender not null or still connected)", sender, this, (System.currentTimeMillis() - startTime));
            notifySenderError();
        }
        else if (receiver!=null && receiver.isOpen()) {
            logger.warn("RRRSSS-Cleaned receiver {} - {} after: {}ms FAILED (receiver not null or still connected)", receiver, this, (System.currentTimeMillis() - startTime));
            notifyReceiverError();
        }
        else if (session!=null) {
            logger.warn("RRRSSS-Cleaned session {} - {} after: {}ms FAILED (session not null or still connected)", session, this, (System.currentTimeMillis() - startTime));
            notifyReceiverError();
        }
        else if (operationSucceed) {
            logger.info("RRRSSS-Cleaned client {} after: {}ms {} DONE", this, (System.currentTimeMillis() - startTime), Thread.currentThread().getId(), Thread.currentThread().getName());
        }
        else {
            logger.info("RRRSSS-Cleaned client {} after: {}ms {} FAILED", this, (System.currentTimeMillis() - startTime), Thread.currentThread().getId(), Thread.currentThread().getName());
        }
    }
